...
from ldisalvo_skeesara_vidyaap.helper.constants import TEAM_NAME, \
    VOTING_DISTRICT_TOWNS_NAME, DEMOGRAPHIC_DATA_DISTRICT_HOUSE_NAME, DEMOGRAPHIC_DATA_TOWN_NAME


class demographicDataDistrictHouse(dml.Algorithm):
    ...
    def execute(trial=False):
        ...
        repo.dropCollection(DEMOGRAPHIC_DATA_DISTRICT_HOUSE_NAME)
        repo.createCollection(DEMOGRAPHIC_DATA_DISTRICT_HOUSE_NAME)
        repo[DEMOGRAPHIC_DATA_DISTRICT_HOUSE_NAME].insert_many(records)
        repo[DEMOGRAPHIC_DATA_DISTRICT_HOUSE_NAME].metadata({'complete': True})
        print(repo[DEMOGRAPHIC_DATA_DISTRICT_HOUSE_NAME].metadata())
